...
public class DifficultySelect extends GuiMenu {
	...
	public DifficultySelect(boolean hosting) {
		...
		if (!hosting) {
			...
			creativeModeBtn = (Checkbox) addButton(new Checkbox(TitleMenu.CREATIVE_ID,
				MojamComponent.texts.getStatic("options.creative"), 
				20,
				MojamComponent.GAME_HEIGHT - 24 - 25,
				creative));
	...
	@Override
	public void buttonPressed(ClickableComponent button) {
		if (button instanceof Checkbox && button != creativeModeBtn) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == Options.getAsInteger("key_left")) {
			...
			nextActiveButtonId = bestExistingDifficultyId(activeButtonId + 3, activeButtonId - 6, activeButtonId - 3);
		} else if (e.getKeyCode() == KeyEvent.VK_ENTER || e.getKeyCode() == Options.getAsInteger("key_use")) {
