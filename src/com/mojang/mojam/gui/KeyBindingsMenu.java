...
public class KeyBindingsMenu extends GuiMenu {
	...
	public void keyPressed(KeyEvent e) {
		if (selectedKey != null) {
			...
			{
				if (buttons.get(selectedItem) == back) selectedItem -= 11;
				else if (selectedItem >= 10)selectedItem -= 10;
			} 
				...
				if (selectedItem + 1 < buttons.size()) selectedItem++;
			} 
			else if (e.getKeyCode() == KeyEvent.VK_RIGHT || e.getKeyCode() == Options.getAsInteger("key_right")) {
				if (buttons.get(selectedItem) == back) selectedItem -= 1;
				else if (selectedItem < 10) selectedItem += 10;
			} 
