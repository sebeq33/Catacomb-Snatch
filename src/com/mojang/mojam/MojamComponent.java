...
public class MojamComponent extends Canvas implements Runnable, MouseMotionListener, CommandListener, PacketListener, MouseListener, ButtonListener, KeyListener {
	...
	public static final String GAME_VERSION = "1.0.0-SNAPSHOT personnaly modified by Sebeq33, me :)";
	...
	public static int GAME_WIDTH = 512; //512
	...
	private static Stack<GuiMenu> menuStack = new Stack<GuiMenu>();
	...
	public static void startgame() {
		...
		guiFrame.addWindowListener(newWindowlistener());
		guiFrame.addComponentListener(newComponentAdapter());
	...
	public void stop(boolean exit) {
		if (exit) {		
			...
			addMenu(new ExitMenu(GAME_WIDTH, GAME_HEIGHT, level != null));
		}
	}
	...
	private void init() {
		...
		try {
			...
			else ((SoundPlayer) soundPlayer).updateMute();
	...
	private synchronized void initLevel(GameCharacter character) {
		...
		players[0] = new Player(
				...
				(int)(Math.floor((level.width + 1)/ 2)) * Tile.WIDTH - 16,
	...
	public void run() {
		...
		while (running) {
			...
			if (shouldRender && bs != null && running) {
					bs.show();
			}
	...
	private void addScore(Screen screen) {
		screen.blit(Art.panel_coin, 311, screen.h - 55);
		Font font = player.score == Player.maxScore ? Font.FONT_RED : Font.defaultFont();
		font.draw(screen, texts.money(player.score), 332, screen.h - 52);
	}
	
	private void addKeyDisplay(Screen screen) {
		screen.blit(Art.pickupKey[3][0], 374, screen.h - 57);
		Font font = player.key == Player.maxKey ? Font.FONT_RED : Font.defaultFont();
		font.draw(screen, texts.keyAmount(player.key), 396, screen.h - 52);
	}

	private void renderMouse(Screen screen, MouseButtons mouseButtons) {

		if (mouseButtons.mouseHidden) return;
		...
		screen.blit(marker,
				mouseButtons.getX() - crosshairSizeHalf - 2,
				mouseButtons.getY() - crosshairSizeHalf - 2);
	}
	...
	private void tick() {
		...
		if(this.isFocusOwner() && level != null) {
			if(menuStack.isEmpty()) {
				keys.console.tick();
				if (console.isOpen()) keys.pause.tick();
				
				if(keys.console.wasPressed() || (console.isOpen() && keys.pause.wasPressed())) {
					console.toggle();
					paused = !paused;
				}
			}
		...
		else mouseButtons.mouseHidden = true; // for levelEditor moves with mouse
		...
		if (createServerState == 1) {
			...
			clearMenus();
			createLevel(TitleMenu.level, TitleMenu.currentGameMode, playerCharacter);
	...
	public void handle(Packet packet) {
		if (packet instanceof StartGamePacket) {
			if (!isServer) {
				...
				createLevel(sgPacker.getLevelFile(), TitleMenu.currentGameMode,
	...
	public void handleAction(int id) {
		...
		case TitleMenu.SELECT_LEVEL_ID:
			addMenu(new LevelSelect(TitleMenu.SELECT_LEVEL_ID));
			break;

		case TitleMenu.SELECT_HOST_LEVEL_ID:
			addMenu(new LevelSelect(TitleMenu.SELECT_HOST_LEVEL_ID));
			break;
			
		case TitleMenu.SELECT_LEVEL_EDITOR:
			addMenu(new LevelSelect(TitleMenu.SELECT_LEVEL_EDITOR));
			break;
		...
		case TitleMenu.RETURN_TO_EDITOR:
			popMenu();// remove level selection
			popMenu();// remove level editor
			addMenu(new LevelEditorMenu());//add a new level editor(the map have been already changed by level select)*/
			break;
	...
	/**
	 * return a value contained between par2 and par3, if it's contained return par1
	 */
	...
	private static WindowListener newWindowlistener() {
		return new WindowAdapter() {
			@Override
			public void windowClosing(WindowEvent winEvt) {
				if (MojamComponent.instance.level != null && menuStack.isEmpty() && !MojamComponent.instance.paused)
				{
					MojamComponent.instance.addMenu(new PauseMenu(GAME_WIDTH, GAME_HEIGHT));
					MojamComponent.instance.paused = true;
					MojamComponent.instance.stop(false);
				}
				else MojamComponent.instance.stop(true);
			}
		};
	}
	
	private static ComponentListener newComponentAdapter() {
		return new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e)
			{
				if (e.getID() == ComponentEvent.COMPONENT_RESIZED)
				{
					...
					/*int scale = MojamComponent.SCALE;
					int contentPaneHeight = guiFrame.getContentPane().getHeight();
					int contentPaneWidth = guiFrame.getContentPane().getWidth();
					
					if (contentPaneWidth >= guiFrame.getMinimumSize().width && contentPaneHeight >= guiFrame.getMinimumSize().height)
					{
						System.out.println(GAME_WIDTH + " ///// " + contentPaneWidth / scale);
						System.out.println(GAME_HEIGHT + " ///// " + contentPaneHeight / scale);
						
						if (contentPaneWidth > contentPaneHeight / 0.75)
						{
							System.out.println(1);
							GAME_HEIGHT = contentPaneHeight / scale;
							GAME_WIDTH = (int) (GAME_HEIGHT / 0.75);
							guiFrame.getContentPane().setSize(GAME_WIDTH * scale, GAME_HEIGHT * scale);
							guiFrame.getContentPane().validate();
							guiFrame.validate();
						}
						else if (contentPaneWidth < contentPaneHeight / 0.75)
						{
							System.out.println(2);
							GAME_WIDTH = contentPaneWidth / scale;
					...
					else
					{
						...
						guiFrame.getContentPane().setSize(GAME_WIDTH * scale, GAME_HEIGHT * scale);
						guiFrame.getContentPane().validate();
						//guiFrame.setSize(contentPaneWidth,contentPaneHeight);
					...
					//guiFrame.getMaximumSize();
					
					JPanel panel = new JPanel(new BorderLayout());
					panel.add(MojamComponent.instance);
					guiFrame.setContentPane(panel);
