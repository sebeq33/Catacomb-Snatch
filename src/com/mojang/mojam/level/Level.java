...
public class Level {
	...
	public boolean spawnSpawner = true;
	...
	public void setMinimapColor(int x, int y, int color) {
		Entity entity = LevelUtils.getNewEntityFromColor(color, 0, 0);
			
		minimap.pixels[x + (y * width)] = 
			(entity == null || entity.minimapColor == -1) ? 
				getTile(x, y).minimapColor : 
				entity.minimapColor;
	}
	
	public int getMinimapColor(int x, int y) {
		return minimap.pixels[x + (y * width)];
	}
	...
	public Set<Entity> getAllEntities(Class<? extends Entity> c) {
		int x0 = 0;
		int x1 = (int) width * Tile.WIDTH;
		int y0 = 0;
		int y1 = (int) height * Tile.HEIGHT;
		
		if (c == null) return getEntities(x0, y0, x1, y1);
		
		return getEntities(x0, y0, x1, y1, c);
	}
	...
	public void addEntity(Entity e) {
		if (e instanceof SpawnerEntity && !spawnSpawner) return;
		e.init(this);
		entities.add(e);
		insertToEntityMap(e);
	}
	
	public boolean checkSpawnableEntityEmplacement(double x , double y){
		final Tile tile = getTile((int) (x / Tile.WIDTH), (int) (y / Tile.HEIGHT));
		
		if (tile instanceof FloorTile) {
			double rx = Tile.WIDTH * 8; // 8 square
			double ry = Tile.HEIGHT * 8;
			
			if (getEntities(new BB(null, x - rx, y - ry, x + rx, y + ry),
					Player.class).size() > 0) return false;
			if (getEntities(new BB(null, x - rx, y - ry, x + rx, y + ry),
					SpawnerEntity.class).size() > 0) return false;
			
			rx = Tile.WIDTH * 4; // 4 square
			ry = Tile.HEIGHT * 4;
			
			if (getEntities(new BB(null, x - rx, y - ry, x + rx, y + ry),
					Turret.class).size() > 0) return false;
			
			rx = Tile.WIDTH /2; // just the current square
			ry = Tile.HEIGHT /2;
			
			if (getEntities(new BB(null, x - rx, y - ry, x + rx, y + ry),
					Building.class).size() > 0) return false;
	...
	public boolean addMob(Mob m, int xTile, int yTile)
	{
		updateDensityList();
		if(spawn && monsterDensity[(int)(xTile/densityTileWidth)][(int)(yTile/densityTileHeight)] < TitleMenu.difficulty.getAllowedMobDensity())
		{
			addEntity(m);
			return true;
		}
	...
	public void updateDensityList()
	{
		for(int x=0;x < monsterDensity.length;x++)
		{
			...
			{
				int entityNumb = getEntities(x*densityTileWidth*Tile.WIDTH,y*densityTileHeight*Tile.HEIGHT,x*(densityTileWidth+1)*Tile.WIDTH,y*(densityTileHeight+1)*Tile.HEIGHT, HostileMob.class).size();
	...
	private void renderTilesAndBases(Screen screen, int x0, int y0, int x1, int y1){
	    ...
	    for (int y = y0; y <= y1; y++) {
	        for (int x = x0; x <= x1; x++) {
				...
				int xt = x - (int)(Math.ceil((width + 1) / 2)) + ((baseOneTileWidth-(baseOneTileWidth%2))/2) + (baseOneTileWidth%2);
	            ...
	            if (xt >= 0 && yt >= 0 && xt < baseOneTileWidth && yt < baseOneTileHeight && (isNotBaseRailTile(x) || yt < baseOneTileHeight-1)) {
	                screen.blit(playerBaseOne[xt][yt], x * Tile.WIDTH, y * Tile.HEIGHT);
	                continue;
	            }
	            
	            // if we are in the center area (4*7 Tiles): draw player bases
				xt = x - (int)(Math.ceil((width + 1) / 2)) + ((baseZeroTileWidth-(baseZeroTileWidth%2))/2) + (baseZeroTileWidth%2);
	...
	private void renderPanelAndMinimap(Screen screen, int x0, int y0){
        ...
        screen.blit(Art.panel, screen.w - Art.panel.w, screen.h - 80);
        screen.blit(displaymap, screen.w - 83, screen.h - 75);
	}
	...
	public void reveal(int x, int y, int radius, boolean forceReveal) {
		for (int i = 0; i < radius * 2 + 1; i++) {
			revealLine(x, y, x - radius + i, y - radius, radius, forceReveal);
			revealLine(x, y, x - radius + i, y + radius, radius, forceReveal);
			revealLine(x, y, x - radius, y - radius + i, radius, forceReveal);
			revealLine(x, y, x + radius, y - radius + i, radius, forceReveal);
		}
	}

	private void revealLine(int x0, int y0, int x1, int y1, int radius, boolean forceReveal) {
		for (int i = 0; i <= radius; i++) {
			...
			if (tile instanceof WallTile && !forceReveal) return;
