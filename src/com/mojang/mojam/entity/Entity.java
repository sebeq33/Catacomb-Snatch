...
public abstract class Entity implements BBOwner {
	...
	protected boolean move(double xa, double ya) {
		...
		boolean moved = !(physicsSlide || xa == 0 || ya == 0);
		if (this instanceof Bullet) System.out.println("\n0 : " +moved +" ///// xa : " + xa + " ///// ya : " + ya + " ///// physicsSlide : " + physicsSlide);
		
		if (moved)
		{
			if (!removed) moved &= partMove(bbs, xa, 0);
			if (this instanceof Bullet) System.out.println("Pass 1 : " +moved);
			if (!removed) moved &= partMove(bbs, 0, ya);
		}
		else
		{
			if (!removed) moved |= partMove(bbs, xa, 0);
			if (this instanceof Bullet) System.out.println("Pass 2 : " +moved);
			if (!removed) moved |= partMove(bbs, 0, ya);
		}
		
		if (this instanceof Bullet) System.out.println("1 : " +moved);
		return moved;
