...
public abstract class Mob extends Entity {
	...
	public void doRegenTime() {
		if (this.REGEN_HEALTH && hurtTime <= 0 && health < maxHealth && --healingTime <= 0) {
			this.healingTime = this.REGEN_INTERVAL;
			this.onRegenTime();
		}
		// else DO NOTHING -> REGEN_HEALTH Apply to all health based buff, so prefer REGEN_AMOUNT = 0 for entity that can apply Poison, and Healing potion.
	}
	...
	public void die() {
		...
		if (dropHeart > 0 && TurnSynchronizer.synchedRandom.nextInt(dropHeart) == 0) {
				level.addEntity(new LootItem(pos.x, pos.y, Math.cos(dir), Math.sin(dir), LootItem.PICKUP_HEART));
		}
            ...
            if (!(this instanceof Player)){
            	...
            	if (!player.isImmortal)
	                ...
	                if (player.isCarrying()){
	                    ...
	                    if(player.carrying instanceof Harvester){
	                        animation.setHarvester();
	                    }
	                    level.addEntity(animation);
    ...
    public boolean onWater() {
        int x=(int)(pos.x/Tile.WIDTH);
        int y=(int)(pos.y/Tile.HEIGHT);
        if (level.getTile(x, y) instanceof WaterTile && !flying) {
            return true;
        }
        return false;
    }
